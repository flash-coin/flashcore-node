...
var bitcore = require('flashcore-lib');
var zmq = require('zeromq');
var async = require('async');
var LRU = require('lru-cache');
var BitcoinRPC = require('flashcoind-rpc');
...
Bitcoin.prototype.getAPIMethods = function() {
  var methods = [
    ...
    ['getAddressMempool', this, this.getAddressMempool, 1]    // WL: Add to calculate Balance
  ];
  return methods;
};
...
Bitcoin.prototype._loadSpawnConfiguration = function(node) {
  ...
  $.checkArgument(this.options.spawn, 'Please specify "spawn" in flashcoind config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in flashcoind config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in flashcoind config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './flashcoin.conf');
...
Bitcoin.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    ...
    '"txindex" option is required in order to use transaction query features of flashcore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of flashcore-rpc-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of flashcore-rpc-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to flashcoind from flashcore-rpc. ' +
      'Please add "server=1" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubrawtx,
    '"zmqpubrawtx" option is required to get event updates from flashcoind. ' +
      'Please add "zmqpubrawtx=tcp://127.0.0.1:<port>" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubhashblock,
    '"zmqpubhashblock" option is required to get event updates from flashcoin. ' +
  ...
  $.checkState(
    ...
    '"zmqpubrawtx" and "zmqpubhashblock" are expected to the same host and port in flashcoin.conf'
  );

  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that flashcoind is undergoing a reindex. ' +
             ...
             'of flashcore-node services will start.');
    node._reindex = true;
  }
};
...
Bitcoin.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === bitcore.Networks.testnet) {
    networkPath = 'testnet3/flashcoin.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/flashcoin.conf';
    }
  }
  return networkPath;
};
...
Bitcoin.prototype._stopSpawnedBitcoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/flashcoind.pid';
...
/**
 * Will give the txs from Mempool
 * @param {Function} callback
 */
Bitcoin.prototype.getAddressMempool = function(addresses, callback) {
  var self = this;
  self.client.getAddressMempool({'addresses': addresses}, function(err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));

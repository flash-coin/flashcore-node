...
function add(options, done) {
  ...
  var bitcoreConfigPath = path.resolve(configPath, 'flashcore-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(bitcoreConfigPath) || !fs.existsSync(packagePath)) {
    return done(
      new Error('Directory does not have a flashcore-node.json and/or package.json file.')
    );
  }
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      addService(configPath, service, function(err) {
        ...
        // add service to flashcore-node.json

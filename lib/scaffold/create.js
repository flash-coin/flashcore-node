...
var BASE_PACKAGE = {
  ...
  readme: 'README.md',
  dependencies: {
    'flashcore-lib': '^' + bitcore.version,
    'flashcore-node': version
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/flashcore-node.json', configJSON);
      if (!isGlobal) {
 ...
 * @param {String} options.dirname - The name of the flashcore node configuration directory
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the flashcore-node directory and configuration
